text="""99lbqpxzzlbtvkmfrvrnmcxttseven
q7cnfslbtpkvseven
...
digits = '123456789'
str2digit = {'one': '1', 'two': '2', 'three': '3', 'four': '4', 'five': '5', 'six': '6', 'seven': '7', 'eight': '8', 'nine': '9'}
lines = text.splitlines()

def get_num(line, forward=True):
    iteration = range(len(line)) if forward else range(len(line) - 1, -1, -1)
    for i in iteration:
        char = line[i]
        if char in digits:
            return char
        
        for name, num in str2digit.items():
            if line[i:i+4] == name:
                return num

def number(line):
    first = get_num(line)
    last = get_num(line, forward=False)
    return int(first+last)

print(sum(map(number, lines)))